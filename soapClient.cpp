...
SOAP_SOURCE_STAMP("@(#) soapClient.cpp ver 2.7.10 2008-05-20 02:00:07 GMT")
...
SOAP_FMAC5 int SOAP_FMAC6 soap_call_ns__test(struct soap *soap, const char *soap_endpoint, const char *soap_action, char *a, char **result)
{	struct ns__test soap_tmp_ns__test;
	struct ns__testResponse *soap_tmp_ns__testResponse;
	if (!soap_endpoint)
		soap_endpoint = "http://192.168.0.131:1234";
	soap->encodingStyle = "";
	soap_tmp_ns__test.a = a;
	soap_begin(soap);
	soap_serializeheader(soap);
	...
	*result = NULL;
	if (soap_begin_recv(soap)
	 ...
	 || soap_body_begin_in(soap))
		return soap_closesock(soap);
	soap_tmp_ns__testResponse = soap_get_ns__testResponse(soap, NULL, "ns:testResponse", "");
	if (soap->error)
	{	if (soap->error == SOAP_TAG_MISMATCH && soap->level == 2)
			return soap_recv_fault(soap);
		return soap_closesock(soap);
	...
	if (soap_body_end_in(soap)
	 ...
	 || soap_end_recv(soap))
		return soap_closesock(soap);
	if (result && soap_tmp_ns__testResponse->result)
		*result = *soap_tmp_ns__testResponse->result;
	return soap_closesock(soap);
}
